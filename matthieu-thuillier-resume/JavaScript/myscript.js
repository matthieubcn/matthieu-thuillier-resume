...
const alertDownload = () => {
  alert(
    `Thanks for downloading my CV - you'll be redirected to a new tab soon`
  );
...
if (document.cookie.indexOf("cookies-accepted=true") === -1) {
  ...
  document
    .querySelector("#accept-cookies-button")
...
const setBgTwo = () => {
  let changeColorTwo = document.querySelectorAll(
    ".skills-progress-intermediate"
  );
  const randomColor = Math.floor(Math.random() * 16777215).toString(16);
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    let message = document.getElementById("message").value;
    console.log(name, email, company, companySize, message);
